...
static string const g_strNoAppendMetadata = "no-append-metadata";
...
General Information)").c_str(),
	...
	metadataOptions.add_options()
		(
			g_strNoAppendMetadata.c_str(),
			"Do not append metadata to the end of the bytecode"
		)
		(
...
void CommandLineParser::processArgs()
{
	...
	map<string, set<InputMode>> validOptionInputModeCombinations = {
		...
		{g_strNoAppendMetadata, {InputMode::Compiler, InputMode::CompilerWithASTImport}},
	...
	if (m_args.count(g_strNoAppendMetadata))
	{
		if (m_args.count(g_strMetadataHash))
		{
			// TODO Should we allow setting both of them at the same time?
		}
